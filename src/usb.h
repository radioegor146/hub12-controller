...
typedef void (*USBEndpointHandler)(uint8_t* buffer, uint16_t length);
...
typedef struct {
  ...
  const USBInterfaceDescriptor* interface_descriptor;
  const USBConfigurationDescriptor* config_descriptor;
  const uint8_t* lang_descriptor;
  const uint8_t** descriptor_strings;

  USBEndpointConfiguration endpoints[USB_NUM_ENDPOINTS];
